...
static int execute_fork(SimpleCommand *cmd_s, int background) {
    subprocess_t *dummy_ptr;
    int status;
    char **command = cmd_s->command_tokens;
    pid_t pid, wpid;
    dummy_ptr = add_subprocess(pid, getpgid(pid), command[0]);
    pid = fork();
    if (pid == 0) {
        /* child */
        ...
        complete_subprocess(dummy_ptr, pid, getpgid(pid));
        setpgid(pid, pid);
        if (background == 0) {
    ...
    switch (cmd->command_type) {
        ...
        case C_PIPE: {
            ...
            for (int i = 0; i < cmd_count; i++) {
                ...
                dummy = add_subprocess(pid, getpgid(pid), tokens[0]);
                pid = fork();
                ...
                if (pid == -1) {
                    ...
                    complete_subprocess(dummy, pid, getpgid(pid));
                    pids[i] = pid;
                    /**
            ...
            for (int i = 0; i < cmd_count; i++) {
                ...
                update_subprocess(pids[i], state);
            }
